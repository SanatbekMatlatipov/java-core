package uz.adventure.challenge;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        tempExit = new HashMap<>();
        tempExit.put("W", 1);
        locationMap.put(3, new Location(3, "You are inside a building, a well house for a small spirit", tempExit));
        ...
        while (true) {
            ...
            if (direction.length() > 1) {
                getDirectionFromInputText(direction);
            }
    ...
    private static String getDirectionFromInputText(String input) {
        String[] inputs = input.replace(",", "").replace(".", "").split(" ");
        for (String element : inputs) {
            if (element.equalsIgnoreCase("east")) {
                return "E";
            }
            if (element.equalsIgnoreCase("south")) {
                return "S";
            }
            if (element.equalsIgnoreCase("west")) {
                return "W";
            }
            if (element.equalsIgnoreCase("north")) {
                return "N";
            }
        }
        return null;
